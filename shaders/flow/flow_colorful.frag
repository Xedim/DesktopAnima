...
void main() {
    ...
    vec3 offsetTrail = texture(u_trailTex, offsetUV / u_resolution).rgb * u_trailDecay;

    vec3 pixelColor = vec3(
        fract(sin(dot(uv_px + u_seed, vec2(12.9898,78.233))) * 43758.5453),
        fract(sin(dot(uv_px + u_seed, vec2(93.9898,67.345))) * 24634.6345),
        fract(sin(dot(uv_px + u_seed, vec2(54.5345,23.234))) * 56445.2345)
    );
    float inject = step(0.995, hash(uv_px + u_seed));
    vec3 newColor = offsetTrail * u_trailDecay + pixelColor * inject * u_intensity;
