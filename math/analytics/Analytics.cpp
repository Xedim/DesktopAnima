...
namespace Analytics {
    ...
    // ---------------- Differential ----------------
    [[nodiscard]] Real derivative(const Utils::Function1D& f, Real x, Real h, StabPolicy policy) {
    ...
    [[nodiscard]] Real derivativeX(const Utils::Function2D& f, Real x, Real y_fixed, Real h, StabPolicy policy) {
    ...
    [[nodiscard]] Real derivativeY(const Utils::Function2D& f, Real x_fixed, Real y, Real h, StabPolicy policy) {
    ...
    // ---------------- Norm ----------------
    [[nodiscard]] Real L2_norm(const Utils::Function1D& f, Real a, Real b, int n, StabPolicy policy) {
    ...
    // ---------------- Properties ----------------
    [[nodiscard]] bool isLocallyEvenFunction(const Utils::Function1D& f, Real x, Real eps, StabPolicy policy) {
    ...
    [[nodiscard]] bool isLocallyOddFunction(const Utils::Function1D& f, Real x, Real eps, StabPolicy policy) {
        if (eps <= Real{0}) return false;
        Real fx  = Utils::evalStable(f, x, LO, HI, policy);
        Real fmx = Utils::evalStable(f, -x, LO, HI, policy);
        return Utils::isFiniteNum(fx) && Utils::isFiniteNum(fmx) && std::abs(fmx + fx) < eps;
    ...
    [[nodiscard]] bool isPeriodic(const Utils::Function1D& f, Real x, Real T, Real eps, StabPolicy policy) {
    ...
    [[nodiscard]] bool isLocallyIncreasing(const Utils::Function1D& f, Real x1, Real x2, Real eps, StabPolicy policy) {
    ...
    [[nodiscard]] bool isLocallyDecreasing(const Utils::Function1D& f, Real x1, Real x2, Real eps, StabPolicy policy) {
        if (eps <= Real{0} || x2 < x1) return false;
        Real f1 = Utils::evalStable(f, x1, LO, HI, policy);
        Real f2 = Utils::evalStable(f, x2, LO, HI, policy);
    ...
    [[nodiscard]] bool isFiniteAt(const Utils::Function1D& f, Real x, StabPolicy policy) {
        Real y = Utils::evalStable(f, x, LO, HI, policy);
        return Utils::isFiniteNum(y);
    }

    [[nodiscard]] bool isLocallyConvex(const Utils::Function1D& f, Real x, Real h, StabPolicy policy) {
    ...
    [[nodiscard]] bool isLocallyConcave(const Utils::Function1D& f, Real x, Real h, StabPolicy policy) {
        if (h <= Real{0}) return false;
        Real fxh       = Utils::evalStable(f, x + h, LO, HI, policy);
        Real f_x_minus_h = Utils::evalStable(f, x - h, LO, HI, policy);
        Real fx        = Utils::evalStable(f, x, LO, HI, policy);
    ...
    [[nodiscard]] bool isNonNegative(const Utils::Function1D& f, Real x, StabPolicy policy) {
        Real fx = Utils::evalStable(f, x, Real{0}, HI, policy);
        return Utils::isFiniteNum(fx) && fx >= Real{0};
    }

    [[nodiscard]] bool isContinuous(const Utils::Function1D& f, Real x, Real h, Real eps, StabPolicy policy) {
